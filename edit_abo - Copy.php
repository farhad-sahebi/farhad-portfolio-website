...
$sql = "SELECT * FROM home_about WHERE ID = '$id'";
                ...
                <div class="form-control">
                    <?php if ($result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            ...
                            <textarea id="description" name="caption_text" class="form-control" rows="1" placeholder="Write the info"><?php echo htmlspecialchars($row['Text']); ?></textarea>

                            <label class="form-label" style="margin-top:18px;">Enter The Title</label>
                            <input type="text" class="form-control" placeholder="Slide Title" name="caption_title" value="<?php echo htmlspecialchars($row['Title']); ?>">

                            <div>
                                <img src="<?php echo htmlspecialchars($row['picture']); ?>" style="width: 200px; height: auto; margin-top: 20px;">
                            </div>
...
if (isset($_POST['btn'])) {
    ...
    if (!empty($pic)) {
        ...
        $image_url = $row['picture']; // Retain old image if no new image is uploaded
    }

    if ($uploadOk) {
        $update = $conn->query("UPDATE home_about SET Title='$tiel', Text='$text', picture='$targetFile' WHERE id='$id'");
